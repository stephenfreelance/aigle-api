...
SHP_ZIP_URL = 'http://osm13.openstreetmap.fr/~cquest/openfla/export/departements-20180101-shp.zip'
SHP_FILE_NAME = 'departements-20180101.shp'

ADDITIONAL_JSON_URL = 'https://www.data.gouv.fr/fr/datasets/r/7b4bc207-4e66-49d2-b1a5-26653e369b66'
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        temp_dir, file_path = download_file(
            url=SHP_ZIP_URL, file_name='regions.zip')
        ...
        # additional data
        additional_data: List[AdditionalInfos] = download_json(
            url=ADDITIONAL_JSON_URL)

        regions = GeoRegion.objects.filter(
            name__in=[normalize(data['region_name'])
                      for data in additional_data]
        ).all()

        region_name_region_map = {
        ...
        for data_item in additional_data:
            ...
            if not region:
                raise CommandError(f'Region was not found in database: {data_item['region_name']}. Did you import regions?')
        ...
        for feature in shape.shapeRecords():
            ...
            geometry = GEOSGeometry(json.dumps(
                feature.__geo_interface__['geometry']))

            # special case for 69
            code_insee_simplified = '69' if properties['code_insee'].startswith('69') else properties['code_insee']
        ...
        GeoDepartment.objects.bulk_create(
            departments
        )
